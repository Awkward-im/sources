...
//  function ProgIDFromCLSID(para:PCLSID; out _para2:POLESTR):HRESULT;stdcall; external  'ole32.dll' name 'ProgIDFromCLSID';
  ...
  function  CoInitialize(_para1:pointer):HRESULT;stdcall; external  'ole32.dll' name 'CoInitialize';
  ...
  procedure CoTaskMemFree(_para1:pointer);stdcall; external  'ole32.dll' name 'CoTaskMemFree';
...
initialization
  ...
  if Ole32Dll<>0 then
    begin
      CoInitializeEx        :=TCoInitializeExProc        (GetProcAddress(Ole32Dll,'CoInitializeEx'));
      CoAddRefServerProcess :=TCoAddRefServerProcessProc (GetProcAddress(Ole32Dll,'CoAddRefServerProcess'));
      CoReleaseServerProcess:=TCoReleaseServerProcessProc(GetProcAddress(Ole32Dll,'CoReleaseServerProcess'));
      CoResumeClassObjects  :=TCoResumeClassObjectsProc  (GetProcAddress(Ole32Dll,'CoResumeClassObjects'));
      CoSuspendClassObjects :=TCoSuspendClassObjectsProc (GetProcAddress(Ole32Dll,'CoSuspendClassObjects'));
    end;
