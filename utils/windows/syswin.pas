...
function GetEXEbyWnd(w:HWND; out dst:PWideChar):PWideChar; overload;
function GetEXEbyWnd(w:HWND; out dst:PAnsiChar):PAnsiChar; overload;
...
function GetEXEbyWnd(w:HWND; out dst:PWideChar):PWideChar;
var
  hProcess:THANDLE;
  ProcID:dword;
...
begin
  ...
  begin
    ...
    begin
      ModuleName[0]:=#0;
      if GetModuleFilenameExW(hProcess,0,ModuleName,SizeOf(ModuleName))=0 then
        GetProcessImageFileNameW(hProcess,ModuleName,SizeOf(ModuleName));
...
function GetEXEbyWnd(w:HWND; out dst:PAnsiChar):PAnsiChar;
var
  hProcess:THANDLE;
  ProcID:dword;
...
begin
  ...
  begin
    ...
    begin
      ModuleName[0]:=#0;
      if GetModuleFilenameExA(hProcess,0,ModuleName,SizeOf(ModuleName))=0 then
        GetProcessImageFileNameA(hProcess,ModuleName,SizeOf(ModuleName));
...
function GetName(param:pointer):int_ptr; //stdcall;
const
  BufSize = $800;
