...
begin
  ...
  if idx<0 then
    ...
    result:=SendMessage(cb,CB_SETITEMDATA,idx,data);
end;
...
begin
  wnd:=GetDlgItem(Dialog,id);
  if wnd<>0 then
    result:=CB_AddStrData(wnd,astr,data,idx)
  else
    result:=0;
end;
...
begin
  wnd:=GetDlgItem(Dialog,id);
  if wnd<>0 then
    result:=CB_AddStrDataW(wnd,astr,data,idx)
  else
    result:=0;
end;
