...
procedure InsertString(wnd:HWND;num:dword;astr:PAnsiChar);
var
  buf:array [0..127] of WideChar;
begin
  SendMessageW(wnd,CB_SETITEMDATA,
      SendMessageW(wnd,CB_ADDSTRING,0,
        lparam({$IFDEF Miranda}TranslateW{$ENDIF}(FastAnsiToWideBuf(astr,@buf)))),
      num);
end;
...
const
  aIcoBtn : array [0..3] of tIcoBtn = (
    (idc:IDC_DATA_NEW   ; idi:'IDI_NEW'   ; txt:'New'   {$IFDEF Miranda}; aci:'ACI_New'   {$ENDIF}),
    (idc:IDC_DATA_DELETE; idi:'IDI_DELETE'; txt:'Delete'{$IFDEF Miranda}; aci:'ACI_Delete'{$ENDIF}),
    (idc:IDC_DATA_UP    ; idi:'IDI_UP'    ; txt:'Up'    {$IFDEF Miranda}; aci:'ACI_Up'    {$ENDIF}),
    (idc:IDC_DATA_DOWN  ; idi:'IDI_DOWN'  ; txt:'Down'  {$IFDEF Miranda}; aci:'ACI_Down'  {$ENDIF})
  );
...
begin
  ...
  for i:=0 to 3 do
    RegisterIcon(sid,aIcoBtn[i].idi,aIcoBtn[i].aci,aIcoBtn[i].txt);
{
...
begin
  ...
  for i:=0 to 3 do
  begin
   ti.uId     :=GetDlgItem(Dialog,aIcoBtn[i].idc);
   ti.lpszText:={$IFDEF Miranda}TranslateW{$ENDIF}(aIcoBtn[i].txt);
   {$IFDEF Miranda}
   SetButtonIcon(ti.uId,aIcoBtn[i].aci);
   {$ELSE}
    SendMessageW(ti.uId, BM_SETIMAGE, IMAGE_ICON,
      LoadImage(hInstance,aIcoBtn[i].idi,IMAGE_ICON,16,16,0));
   {$ENDIF}
...
begin
  ...
  OldLVProc:=TWNDPROC(SetWindowLongPtrW(list,GWL_WNDPROC,long_ptr(@NewLVProc)));
...
begin
  ...
  p:=storage^.SectionList[namespace];
  i:=0;
  if p<>nil then
    while p^<>#0 do
    begin
      CB_AddStrData(wnd,p,UIntPtr(storage^.Sections[namespace,p]), i);
...
{$ENDIF}
      ...
      if lParam<>0 then
      begin
        ls:=pIniSection(lParam);
        SetDlgItemTextA(Dialog,IDC_HLP_NAME,ls^.Name);

        SetDlgItemTextA(Dialog,IDC_HLP_PLUGIN,ls^.Key['plugin']);

        FastAnsiToWide(ls^.ReadStr('descr','Undefined'),tmp);
...
{$ENDIF}
      storage:=CreateIniFile(API_STRUCT_FILE);
      if storage<>nil then
...
{$ELSE}
      ...
      wnd:=GetDlgItem(Dialog,IDC_DATA_EDTN);
      ...
      wnd:=GetDlgItem(Dialog,IDC_DATA_INFO);
      GetWindowRect(wnd,rc1);
      y:=(rc.bottom-rc.top)-(rc1.bottom-rc1.top)-8;
      SetWindowPos(wnd,0,rc1.left-rc.left,y,0,0,
          SWP_NOSIZE or SWP_NOZORDER);

      wnd:=GetDlgItem(Dialog,IDC_DATA_PASTE);
      GetWindowRect(wnd,rc1);
      SetWindowPos(wnd,0,rc1.left-rc.left,y-(rc1.bottom-rc1.top)-8,0,0,
          SWP_NOSIZE or SWP_NOZORDER);

      wnd:=GetDlgItem(Dialog,IDC_DATA_TMPL);
      GetWindowRect(wnd,rc1);
      y:=(rc.bottom-rc.top)-(rc1.bottom-rc1.top)-8;
      SetWindowPos(wnd,0,0,y,rc1.right-rc1.left,rc1.bottom-rc1.top,
          {SWP_NOMOVE or }SWP_NOZORDER);
