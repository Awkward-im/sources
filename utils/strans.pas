...
function GetOneElement(txt:PAnsiChar;out res:tOneElement;
                       SizeOnly:boolean;num:integer=0):integer;
procedure FreeElement(var element:tOneElement);

//----- Execute -----

function MakeStructure(txt:PAnsiChar;aparam,alast:pointer; isNumber:boolean=true):pointer;
...
type
  // IntPtr = to use aligned structure data at start
...
function AdjustSize(var summ:UIntPtr;eleadjust:integer;adjust:integer):integer;
var
  rest,lmod:cardinal;
begin
...
//          if pc^=' ' then inc(pc);
  ...
  case result of
    SST_LAST,
    SST_PARAM : begin res.len:=SizeOf(UIntPtr); res.align:=SizeOf(UIntPtr); end;
    ...
    SST_NATIVE: begin res.len:=SizeOf(UIntPtr); res.align:=SizeOf(UIntPtr); end; // SizeOf(NativeInt)
...
//    if p<>nil then p^:=#0;
  ...
  addsize:=SizeOF(TStructResult)+SizeOF(tShortTemplate)*amount+SizeOf(cardinal);
  ...
  res:=PByte(PAnsiChar(tmpl)+addsize-SizeOf(TStructResult)-SizeOf(cardinal));
  pcardinal(res)^:=amount; inc(res,SizeOf(cardinal));
...
{$IFDEF Miranda}
      if isNumber then
        pLast:=IntToStr(PWideChar(@buf),UIntPtr(alast))
      else
      ...
      value:=ParseVarString(valuein,UIntPtr(aparam),UIntPtr(pLast));
      mFreeMem(valuein);
      case element.etype of
...
{$ENDIF}
    ...
    AdjustSize(UIntPtr(res),element.align,align);

    tmpl^.etype :=element.etype;
    tmpl^.flags :=element.flags;
    tmpl^.offset:=UIntPtr(res)-UIntPtr(result);

    case element.etype of
      SST_LAST: begin
        PUIntPtr(res)^:=UIntPtr(alast);
      end;
      SST_PARAM: begin
        PUIntPtr(res)^:=UIntPtr(aparam);
      end;
      SST_BYTE: begin
      ...
      SST_DWORD: begin
        pcardinal(res)^:=element.value;
      end;
      SST_QWORD: begin
        pint64(res)^:=element.value;
      end;
      SST_NATIVE: begin
        PIntPtr(res)^:=element.value;
      end;
      SST_BARR: begin
...
//!!!!! variables script gives unicode, need to recognize it
    ...
    if (element.etype=SST_BPTR) or (element.etype=SST_WPTR) then
      inc(IntPtr(res),SizeOf(pointer))
    else
      inc(IntPtr(res),element.len);
...
function GetResultPrivate(var struct;atype:pinteger=nil;alen:pinteger=nil):UIntPtr;
var
...
begin
  ...
  case ltype of
    ...
    SST_DWORD : result:=pcardinal(PAnsiChar(struct)+loffset)^;
    SST_QWORD : result:=puint64  (PAnsiChar(struct)+loffset)^;
    SST_NATIVE: result:=PUIntPtr (PAnsiChar(struct)+loffset)^;

    SST_BARR: result:=UIntPtr(PAnsiChar(struct)+loffset); //??
    SST_WARR: result:=UIntPtr(PAnsiChar(struct)+loffset); //??

    SST_BPTR: result:=PUIntPtr(PAnsiChar(struct)+loffset)^; //??
    SST_WPTR: result:=PUIntPtr(PAnsiChar(struct)+loffset)^; //??
  else
    result:=0;
  end;
end;
...
begin
  ...
  case code of
    SST_BYTE,SST_WORD,SST_DWORD,
    SST_QWORD,SST_NATIVE: begin
      StrDupW(result,IntToStr(PWideChar(@buf),data));
    end;
...
var
  ...
  tmp:pointer;
begin
  tmp:=pointer(PAnsiChar(struct)-SizeOF(TStructResult)-SizeOf(cardinal));
  num:=pcardinal(tmp)^;
  tmpl:=pointer(PAnsiChar(tmp)-num*SizeOf(tShortTemplate));
  lmod:=UIntPtr(tmpl) mod SizeOf(pointer);
  ...
  repeat
    case tmpl^.etype of
      SST_BPTR,SST_WPTR: begin
        //??
        value:=PAnsiChar(PIntPtr(PAnsiChar(struct)+tmpl^.offset)^);
{$IFDEF Miranda}
