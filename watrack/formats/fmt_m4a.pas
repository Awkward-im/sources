...
function ReadM4A(Info:UIntPtr):boolean;
...
procedure ReadAtom(var f:file; var atom:mp4Atom);
begin
...
procedure ReadProp(var p:pbyte; Info, aField:UIntPtr);
var
  len:integer;
  ltmp:PAnsiChar;
  c:byte;
begin
  ...
  c:=p^;
  p^:=0;
  WATSetStr(Info,aField, ltmp, CP_UTF8);
  p^:=c;
end;
      ...
      repeat
        ...
        begin
          if pmvhd(p)^.TimeScale<>0 then
            WATSet(Info,siLength, BSwap(pmvhd(p)^.Duration) div BSwap(pmvhd(p)^.TimeScale));
        end;
        ...
        begin
          ...
          begin
            ...
            repeat
              GetAtom(p,atom);
              if      atom.name=atm_nam  then ReadProp(p, Info, siTitle)
              else if atom.name=atm_ART  then ReadProp(p, Info, siArtist)
//            else if atom.name=atm_wrt  then ReadProp(p, Info, siTitle)
              else if atom.name=atm_alb  then ReadProp(p, Info, siAlbum)
              else if atom.name=atm_day  then ReadProp(p, Info, siYear)
              else if atom.name=atm_cmt  then ReadProp(p, Info, siComment)
//            else if atom.name=atm_gen  then ReadProp(p, Info, siGenre)
              else if atom.name=atm_gnre then WATSetStr(Info,siGenre, GenreName(ReadInt(p)-1), CP_ACP)
              else if atom.name=atm_trkn then WATSet(Info,siTrack, ReadInt(p))
              else
        ...
        begin
          ...
          begin
            ...
            begin
              WATSet(Info,siWidth , swap(pstsd(p)^.Width));
              WATSet(Info,siHeight, swap(pstsd(p)^.Height));
              WATSet(Info,siCodec , pstsd(p)^.DataFormat);
            end;
          end
          // audio props
          ...
          begin
            ...
            if SetTree(atom,p,'stbl.stsd',par)>0 then
            begin
              WATSet(Info,siSamplerate, (BSwap(pastsd(p)^.Samplerate) shr 16) div 1000);
              WATSet(Info,siChannels  , swap(pastsd(p)^.ChannelCount));
            end;
            p:=par;
