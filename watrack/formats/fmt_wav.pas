...
function ReadWAV(Info:UIntPtr):boolean;
...
begin
  ...
  WATSet(Info,siChannels  , fmtchunk.Channels);
  WATSet(Info,siSamplerate, fmtchunk.SampleRate div 1000);
  ...
  if fmtchunk.SampleRate<>0 then
    WATSet(Info,siLength, WPH.total_samples div fmtchunk.SampleRate);

  //??
  if not WATIsEmpty(Info, siLength) then
    WATSet(Info,siSamplerate, tmp*8 div WATGet(Info,siLength) div 1000);
