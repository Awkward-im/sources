...
function ReadWMA(Info:UIntPtr):boolean;
...
procedure ProcessPicture(ptr:PAnsiChar; Info:UIntPtr);
var
  pcw:PWideChar;
  extw:int64;
  aSize:dword;
begin
  ...
  pcw:=SaveTemporaryW(ptr,aSize,pWideChar(@extw));
  WATSetStr(Info,siCover, pcw, CP_UTF16);
  mFreeMem(pcw);
end;

procedure ReadHdrExtended(ptr:PAnsiChar; size:dword; Info:UIntPtr);
var
  buf:PAnsiChar;
  ls:pWideChar;
  cnt,tmp:integer;
...
begin
  ...
  while size>0 do
  begin
    if not WATIsEmpty(Info,siCover) then break;
...
procedure ReadExtended(ptr:PAnsiChar; size:dword; Info:UIntPtr);
var
...
procedure ReadFileProp(ptr:PAnsiChar; Info:UIntPtr);
type
  pFileProp = ^tFileProp;
...
begin
  WATSet(Info,siLength, pFileProp(ptr)^.Play div 10000000);
end;

procedure ReadStreamProp(ptr:PAnsiChar; size:dword; Info:UIntPtr);
type
  pAudio = ^tAudio;
...
begin
  ...
  begin
    WATSet(Info,siChannels  , pAudio(ptr)^.Channels);
    WATSet(Info,siSamplerate, pAudio(ptr)^.Samples div 1000);
    WATSet(Info.siBitrate   , (pAudio(ptr)^.AvgBPS*8) div 1000);
  end
  else if CompareGUID(tmpguid^,ASF_Video_Media) then
  begin
    WATSet(Info,siWidth , pVideo(ptr)^.bitmap.biWidth);  // pVideo(ptr)^.width
    WATSet(Info,siHeight, pVideo(ptr)^.bitmap.biHeight); // pVideo(ptr)^.height
    WATSet(Info,siCodec , pVideo(ptr)^.bitmap.biCompression);
  end
end;

procedure ReadContent(ptr:PAnsiChar; Info:UIntPtr);
type
  pContent = ^tContent;
