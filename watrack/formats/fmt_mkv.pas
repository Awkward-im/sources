...
function ReadMKV(Info:UIntPtr):boolean;
...
begin
  ...
  repeat
    ...
    begin
      ...
      // 2 - bitperpixel?
      tmp:=ptr^; inc(ptr);
      tmp:=tmp+(ptr^ shl 8 ); inc(ptr);
      tmp:=tmp+(ptr^ shl 16); inc(ptr);
      WATSet(Info,siCodec, tmp+(ptr^ shl 24));
      inc(ptr,len-19);
    end
    else if (id=idName) and WATIsEmpty(Info,siTitle) then
    begin
      mGetMem(ls,len+1);
      move(ptr^,ls^,len);
      ls[len]:=#0;
      ...
      WATSet(Info,siWidth, GetInt(ptr,len))
    else if id=idPixelHeight then
      WATSet(Info,siHeight, GetInt(ptr,len))
    else if id=idDefaultDuration then
    begin
      if trktype=1 then
      begin
        tmp:=(GetInt(ptr,len) div 1000);
        if tmp<>0 then
          WATSet(Info,siFPS, 100000000 div tmp);
      end
      else
      begin
        GetInt(ptr,len);
      ...
      WATSet(Info,siSamplerate, round(GetFloat(ptr)) div 1000)
    else if id=idChannels then
      WATSet(Info,siChannels, GetInt(ptr,len))
    else
      inc(ptr,len);
  until pAnsiChar(ptr)>=(PAnsiChar(@buf)+SizeOf(buf));
  WATSet(Info,siLength, trunc(lTotal/(1000000000/scale)));
