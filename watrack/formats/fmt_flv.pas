...
function ReadFLV(Info:UIntPtr):boolean;
...
function ProcessValue(var ptr:PAnsiChar; key:pAnsiChar; Info:UIntPtr):integer;
var
  tmp:int64;
  i,len:integer;
  recode:trecode;
...
begin
  ...
  case code of // v10.1 specification
    ...
    0: begin
      ...
      i:=trunc(recode.d);
      if      StrCmp(key,'duration'     )=0 then WATSet(Info,siLength , i)
      else if StrCmp(key,'totalduration')=0 then WATSet(Info,siLength , i)
      else if StrCmp(key,'width'        )=0 then WATSet(Info,siWidth  , i)
      else if StrCmp(key,'height'       )=0 then WATSet(Info,siHeight , i)
      else if StrCmp(key,'audiodatarate')=0 then WATSet(Info,siBitrate, i)
      else if StrCmp(key,'framerate'    )=0 then WATSet(Info,siFPS    , trunc(recode.d*100))
      else if StrCmp(key,'audiosize'    )=0 then
      begin
        if WATIsEmpty(Info,siBitrate) and not WATIsEmpty(Info,siLength) then
          WATSet(Info,siBitrate, trunc((recode.d*8)/(WATGet(Info,siLength)*1000)));
      end
    ...
    // Boolean, UI8
    1: begin
      if StrCmp(key,'stereo')=0 then WATSet(Info,siChannels, ORD(ptr^)+1);
    ...
    2: begin
      ...
      begin
        move(ptr^,value[0],i);
        value[i]:=#0;
        WATSetStr(Info,siYear, @value, CP_ACP);
      end;
  ...
  begin
    ...
    begin
      ...
      case StrmHdr.TagType of
        ...
        FLV_AUDIO: begin
          WATSet(Info,siChannels, (ord(p^) and 1)+1);
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                // checking for video
                if not WATIsEmpty(Info,siCodec) then
                  FLVHdr.flags:=FLVHdr.flags and not 1;
                // checking for audio
                if not WATIsEmpty(Info,siSamplerate) and not WATIsEmpty(Info,siChannels) then
                  FLVHdr.flags:=FLVHdr.flags and not 4;
